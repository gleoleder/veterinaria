...
function renderHistory(filtered) {
    ...
    container.innerHTML = data.map(item => {
        ...
        const hasPdf = item.pdfUrl && item.pdfUrl.length > 10;
        ...
        return `<div class="history-item">
            ...
            <div class="history-content">
                ...
                <div class="history-attachments">
                    ${hasPdf ? '<a href="' + item.pdfUrl + '" target="_blank" class="attachment-badge pdf-badge">📄 Ver PDF</a>' : ''}
                    ${attachments.map(a => '<a href="' + a.url + '" target="_blank" class="attachment-badge">📎 ' + a.name + '</a>').join('')}
                </div>
            </div>
            <div class="history-actions">
                ${hasPdf ? '<a href="' + item.pdfUrl + '" target="_blank" class="action-btn" title="Descargar PDF">📄</a>' : ''}
                <button class="action-btn" onclick="viewPetHistory(${item.petId})" title="Ver historial completo">📋</button>
            </div>
        </div>`;
    }).join('');
}
